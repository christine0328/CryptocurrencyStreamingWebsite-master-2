#This script is referred to Paul Jeffries's work and used under MIT Lisence
...
js_extern = "https://s3-us-west-2.amazonaws.com/s.cdpn.io/2030371/whale.js"
...
PAIRS = []  # Array containing all currency pairs
E_GDAX = Exchange("GDAX", ["BTC-USD", 
                           "ETH-USD",
                           "LTC-USD"], 0)
for ticker in E_GDAX.ticker:
    obj = Pair(E_GDAX.name, ticker)
    PAIRS.append(obj)
...
def calc_data(pair, range=0.05, maxSize=32, minVolumePerc=0.01, ob_points=60):
    ...
    while i < ob_points:
        ...
        # Calculate cummulative quantity of orders from one address
        num_orders_ask += ask_tbl.loc[
            (ask_tbl[TBL_PRICE] >= last_ask) & (ask_tbl[TBL_PRICE] < current_ask_border), 'address'].count()
        num_orders_bid += bid_tbl.loc[
            (bid_tbl[TBL_PRICE] <= last_bid) & (bid_tbl[TBL_PRICE] > current_bid_border), 'address'].count()

        # Prepare Text
        ask_text = (str(round_sig(current_ask_volume, 3, 0, sig_use)) + currency + " (from " + str(num_orders_ask) +
                     " orders) up to " + str(round_sig(current_ask_border, 3, 0, sig_use)) + symbol)
        bid_text = (str(round_sig(current_bid_volume, 3, 0, sig_use)) + currency + " (from " + str(num_orders_bid) +
                     " orders) down to " + str(round_sig(current_bid_border, 3, 0, sig_use)) + symbol)

        # Save Data
        ob_ask.loc[i - 1] = [current_ask_border, current_ask_volume, num_orders_ask, ask_text]
        ob_bid.loc[i - 1] = [current_bid_border, current_bid_volume, num_orders_bid, bid_text]
        i += 1
    ...
    final_tbl['n_unique_orders'] = fulltbl.groupby(
        TBL_PRICE).address.nunique().astype(int)  # remove the duplicated data
...
def prepare_data(ticker, exchange):
    ...
    result = {
        ...
        'layout': go.Layout(
            ...
            title=("The market price of {} is: {}{} at {}".format(ticker, symbol,
                                                                                str(
                                                                                    marketPrice[combined]),
                                                                                timeStamps[combined])),
            xaxis=dict(title='Order Size', type='log', autotick=True,range=[log10(x_min*0.95), log10(x_max*1.03)]),
            yaxis={'title': '{} Price'.format(ticker),'range':[market_price*0.95, market_price*1.05]},
            hovermode='closest',
...
def serverThread():
    app.run_server(host='127.0.0.2', port=serverPort)
